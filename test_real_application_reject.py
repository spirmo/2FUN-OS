...
concept_id = repo.create_concept(
    creator="TEST_USER",
    concept_code="C_REAL_REJECT_002",
    version="1.0",
...
concept.concept_code = "C_REAL_REJECT_002"
...
result = app.reject_submission(
    concept,
    approval_id=approval_id,
    rejected_by="FOUNDER_REJECT_TEST",
...
assert queue["rejected_by"] == "FOUNDER_REJECT_TEST"
assert queue["reviewed_at"] is not None

assert version["status"] == "REJECTED"
assert version["rejected_by"] == "FOUNDER_REJECT_TEST"
assert version["rejection_reason"] == "TEST_REJECTION_REASON"

assert current["current_status"] == "REJECTED"
assert current["current_version"] == "1.0"
assert current["current_completeness"] == 30

print("REAL APPLICATION REJECTION INTEGRATION TEST: OK")
